...
const login = (req, res) => {
  ...
  db.get('SELECT * FROM users WHERE email = ?', [email], async (err, user) => {
    ...
    logActivity(user.id, 'login', 'user', user.id, `User ${user.name} logged in`, req.ip, 'success');
...
// POST /api/auth/register (admin only or public for demo)
const register = async (req, res) => {
  ...
  db.run(
    ...
    [id, name, email, hash, role, phone || null],
    function (err) {
      if (err) {
...
const getMe = (req, res) => {
  db.get(
    `SELECT u.id, u.name, u.email, u.role, u.phone, u.avatar, u.last_login, u.created_at,
      p.unit_number, p.block, p.floor, l.id as lease_id, l.end_date as lease_end
     FROM users u
